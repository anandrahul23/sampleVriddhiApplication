package com.anand.vriddhi.web.rest;
...
import com.anand.vriddhi.IntegrationTest;
import com.anand.vriddhi.config.EmbeddedKafka;
import com.anand.vriddhi.config.KafkaSseConsumer;
import com.anand.vriddhi.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class SampleVriddhiApplicationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/sample-vriddhi-application-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/sample-vriddhi-application-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/sample-vriddhi-application-kafka/unregister"));
                return;
            }
        }
